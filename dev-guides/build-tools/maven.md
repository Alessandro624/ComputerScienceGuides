...
Questa guida fornisce un'introduzione a Maven, lo strumento di build automation e gestione delle dipendenze più diffuso nell'ecosistema Java. Copre la struttura del progetto, il file pom.xml, le dipendenze, i plugin e i profili.

## Prerequisiti

- JDK >= 8 installato e configurato (variabile `JAVA_HOME`)
- Terminale con accesso a internet per scaricare dipendenze
...
- **Versioni esplicite**: Specifica sempre le versioni delle dipendenze per riproducibilità
- **Dependency Management**: Usa `<dependencyManagement>` per centralizzare le versioni
- **Plugin versions**: Blocca le versioni dei plugin per build riproducibili
- **Evita snapshot in produzione**: Usa versioni release per deploy in produzione
- **Profili per ambiente**: Usa profili Maven per differenziare dev/test/prod
- **Repository locale pulito**: Pulisci periodicamente `~/.m2/repository`

## Riferimenti

- [Documentazione ufficiale Apache Maven](https://maven.apache.org/guides/)
