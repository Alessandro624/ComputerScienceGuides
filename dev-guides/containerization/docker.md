...
Questa guida fornisce una panoramica completa di Docker, lo strumento standard per la containerizzazione delle applicazioni. Copre i comandi essenziali per la gestione di immagini, container, volumi, reti e Docker Compose.

## Prerequisiti

- Docker Desktop (Windows/macOS) o Docker Engine (Linux)
- Terminale/Shell con permessi adeguati
...
Scarica Docker Desktop dal [sito ufficiale](https://www.docker.com/products/docker-desktop/) e segui le istruzioni di installazione.
...
sudo usermod -aG docker $USER  # Aggiunge l'utente al gruppo docker
```
...
- **Usa immagini ufficiali**: Preferisci immagini ufficiali da Docker Hub per maggiore sicurezza
- **Minimizza i layer**: Combina comandi RUN nel Dockerfile per ridurre la dimensione dell'immagine
- **Non eseguire come root**: Usa `USER` nel Dockerfile per eseguire processi come utente non privilegiato
- **Usa .dockerignore**: Escludi file non necessari dal build context
- **Tag specifici**: Usa tag specifici invece di `latest` per riproducibilità
- **Multi-stage builds**: Usa build multi-stage per immagini più leggere
- **Health checks**: Implementa health check per monitorare lo stato dei container
